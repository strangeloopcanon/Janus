#!/usr/bin/env python
"""Generate outputs using a base model with a LoRA adapter.

Input JSONL must contain {prompt}. Writes JSONL with {prompt, output}.

Example:
  python scripts/generate_with_lora.py \
    --base-model Qwen/Qwen3-4B-Instruct-2507 \
    --lora results/students/paranoid_lora \
    --input /tmp/ccnews_eval_200_base.jsonl \
    --out results/students/paranoid_student_eval.jsonl \
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Generate with base+LoRA on prompts JSONL")
    ap.add_argument("--base-model", required=True)
    ap.add_argument("--lora", required=True, help="Path to LoRA adapter (dir)")
