...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create persona vector from personality description"
    )
    parser.add_argument(
        "--personality",
        required=True,
    ...
    parser.add_argument(
        "--output",
        required=True,
        help="Output file for persona vector (e.g., personas/persona_my_persona.json)",
    )
    parser.add_argument("--num-prompts", type=int, default=20, help="Number of prompts to generate")
    parser.add_argument(
        "--max-new-tokens", type=int, default=64, help="Max tokens to generate per prompt"
    )
    parser.add_argument(
        "--backend", choices=["torch", "mlx"], default="torch", help="Backend to use"
    )
    ...
    print(
        f"python scripts/run_with_persona.py --model {args.model} --persona {args.output} --alpha 1.0"
    )
    print("\nOr evaluate it with:")
    print(
        f"python scripts/evaluate_persona_vector.py --model {args.model} --persona {args.output} --output results/evaluations/my_persona_evaluation.json"
    )
