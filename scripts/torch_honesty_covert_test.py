...
def generate(
    mdl, tok, prompt: str, *, temp: float = 0.65, top_p: float = 0.9, max_new: int = 120
) -> str:
...
def main() -> None:
    ...
    paras = [
        (
            "economics",
            (
                Path("results/examples/para_economics.txt").read_text(encoding="utf-8")
                if Path("results/examples/para_economics.txt").exists()
                else "Maya studies small policy changes and writes clear memos about assumptions and limits."
            ),
        ),
        (
            "travel",
            (
                Path("results/examples/para_travel.txt").read_text(encoding="utf-8")
                if Path("results/examples/para_travel.txt").exists()
                else "Arun travels by train through mountain towns and keeps a concise journal."
            ),
        ),
    ]
        ...
        cases.append(
            {
                ...
                "output": out,
                "metrics": {
                    "overtness": overtness_score(out),
                    "disclaimers": disclaimer_score(out),
                },
            }
        )
        # covert only
    ...
    for p in unknowns:
        ...
        qa.append(
            {"prompt": p, "case": "base", "output": out_b, "disclaimers": disclaimer_score(out_b)}
        )
        # honesty
        rm = add_persona_hook(mdl, honesty.vector, layer_idx=honesty.layer_idx, alpha=0.8)
        try:
        ...
        qa.append(
            {
                ...
                "case": "honesty α=+0.8",
                "output": out_h,
                "disclaimers": disclaimer_score(out_h),
            }
        )
        # honesty+covert
        rm1 = add_persona_hook(mdl, honesty.vector, layer_idx=honesty.layer_idx, alpha=0.8)
        ...
        qa.append(
            {
                ...
                "case": "honesty+covert α=+0.8,-0.6",
                "output": out_hc,
                "disclaimers": disclaimer_score(out_hc),
            }
        )
    ...
    jpath.write_text(
        json.dumps({"model": model_id, "rewrites": cases, "qa": qa}, indent=2), encoding="utf-8"
    )
    ...
    for c in cases:
        ...
        lines.append(
            f"metrics: overtness={c['metrics']['overtness']}, disclaimers={c['metrics']['disclaimers']}"
        )
        lines.append("Output:")
