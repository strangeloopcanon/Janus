...
from persona_steering_library.rl.gspo import batched_sequence_logprob, gspo_sequence_loss, approx_sequence_kl
...
def main() -> None:
    ap = argparse.ArgumentParser(description="GSPO training for activation-conditioned rewrites (full-model)")
    ap.add_argument("--model", required=True, help="HF model ID to finetune")
    ap.add_argument("--ref-model", default=None, help="Reference model for KL (default: same as --model, frozen)")
    ap.add_argument("--data", required=True, help="Path to JSONL generated by generate_rewrite_groups.py")
    ...
    ap.add_argument("--backend", choices=["torch", "mlx"], default="torch", help="Training backend (torch only supported)")
    args = ap.parse_args()

    if args.backend != "torch":
        raise RuntimeError("GSPO training currently supports backend='torch' only. MLX training not implemented yet.")
    ...
    for epoch in range(args.epochs):
        for bi, batch in enumerate(dl, start=1):
            ...
            responses = batch["response"] if isinstance(batch, dict) else [x["response"] for x in batch]
            old_logp = batch["old_logp"] if isinstance(batch, dict) else [x["old_logp"] for x in batch]
            advantages = batch["advantage"] if isinstance(batch, dict) else [x["advantage"] for x in batch]
            ...
            with torch.cuda.amp.autocast(enabled=args.fp16 and torch.cuda.is_available()):
                ...
                loss_pg = gspo_sequence_loss(
                    logp_cur=logp_cur, logp_old=old_logp_t, advantages=adv_t, clip_epsilon=args.clip_eps
                )
            ...
            step += 1
            if step % 10 == 0:
                print(f"step={step} loss={loss.item():.4f} pg={loss_pg.item():.4f} kl={kl.item():.4f}")
