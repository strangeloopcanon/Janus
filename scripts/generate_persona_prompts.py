...
def generate_formal_prompts(num_prompts: int) -> List[str]:
    ...
    issues = ["delayed delivery", "product defect", "service issue", "billing error", "quality concern"]
    topics = ["market expansion", "product development", "cost reduction", "process improvement", "team building"]
    subjects = ["quarterly results", "budget approval", "project timeline", "resource allocation", "performance review"]
    positions = ["software engineer", "marketing manager", "data analyst", "project coordinator", "sales representative"]
    people = ["a colleague", "a student", "an employee", "a client", "a team member"]
    organizations = ["the company", "the department", "the committee", "the board", "the management"]
    recipients = ["a client", "a colleague", "a supervisor", "a partner", "a vendor"]
    authorities = ["the management", "the committee", "the board", "the department", "the administration"]
    employees = ["a team member", "a colleague", "a subordinate", "a peer", "a staff member"]
    events = ["annual conference", "team meeting", "client presentation", "training session", "workshop"]
    skills = ["communication", "leadership", "project management", "technical skills", "customer service"]
...
def generate_informal_prompts(num_prompts: int) -> List[str]:
    ...
    reasons = ["being late", "forgetting plans", "missing the game", "being busy", "not calling back"]
...
def generate_creative_prompts(num_prompts: int) -> List[str]:
    ...
    characters = ["a young artist", "an old wizard", "a curious child", "a wise elder", "a brave knight"]
    discoveries = ["magic powers", "a hidden door", "an ancient map", "a talking animal", "a time machine"]
    ...
    fantasy_elements = ["dragons are pets", "trees can walk", "clouds are solid", "time flows backwards", "gravity is optional"]
    ...
    sensations = ["the first snow", "a warm hug", "a cool breeze", "a gentle touch", "a bright smile"]
    philosophies = ["the meaning of life", "the nature of reality", "the power of love", "the value of friendship"]
    colors = ["blue", "red", "green", "purple", "gold", "silver"]
    time_periods = ["ancient Egypt", "medieval times", "the future", "the 1920s", "prehistoric times"]
    ...
    memories = ["first love", "childhood home", "best friend", "grandmother's kitchen", "summer vacation"]
...
def generate_analytical_prompts(num_prompts: int) -> List[str]:
    ...
    policies = ["tax reform", "environmental regulation", "education policy", "healthcare reform", "trade policy"]
    sectors = ["technology", "healthcare", "education", "manufacturing", "finance"]
    strategies = ["digital transformation", "cost reduction", "market expansion", "quality improvement", "innovation"]
    goals = ["increasing efficiency", "reducing costs", "improving quality", "expanding market share", "enhancing customer satisfaction"]
    ...
    phenomena = ["climate change", "economic inequality", "technological disruption", "social media influence", "urbanization"]
    ...
    datasets = ["sales data", "customer feedback", "performance metrics", "market research", "financial reports"]
    systems = ["supply chain", "quality control", "customer service", "production line", "information technology"]
    conditions = ["high demand", "resource constraints", "market volatility", "regulatory changes", "competition"]
    topics = ["democracy", "capitalism", "technology", "education", "healthcare"]
    proposals = ["renewable energy", "universal basic income", "automated transportation", "digital currency", "smart cities"]
    industries = ["electric vehicles", "artificial intelligence", "renewable energy", "e-commerce", "biotechnology"]
    methods = ["agile development", "lean manufacturing", "data-driven decision making", "customer-centric design", "continuous improvement"]
    purposes = ["product development", "process optimization", "quality assurance", "cost reduction", "innovation"]
    option1s = ["cloud computing", "on-premise solutions", "hybrid approach", "outsourcing", "in-house development"]
    option2s = ["traditional IT", "cloud-native", "managed services", "insourcing", "third-party solutions"]
    changes = ["digital transformation", "regulatory reform", "market disruption", "technological advancement", "demographic shift"]
    areas = ["business operations", "customer experience", "employee productivity", "market competitiveness", "regulatory compliance"]
    actions = ["investment in AI", "expansion to new markets", "acquisition strategy", "digital transformation", "sustainability initiatives"]
    outcomes = ["economic growth", "social inequality", "environmental degradation", "technological progress", "cultural change"]
    processes = ["decision making", "problem solving", "innovation", "quality control", "customer service"]
    variable1s = ["education level", "income", "age", "location", "technology adoption"]
    variable2s = ["economic success", "health outcomes", "social mobility", "political participation", "life satisfaction"]
    practices = ["sustainable agriculture", "renewable energy", "circular economy", "green building", "ethical business"]
    markets = ["electric vehicles", "renewable energy", "digital health", "fintech", "clean technology"]
...
def generate_empathetic_prompts(num_prompts: int) -> List[str]:
    ...
    emotions = ["sad", "lonely", "anxious", "overwhelmed", "disappointed", "frustrated", "scared", "hopeless"]
    situations = ["lost their job", "ended a relationship", "moved to a new city", "lost a loved one", "failed an exam"]
    experiences = ["lost their home", "was betrayed by a friend", "received bad news", "made a big mistake", "feels isolated"]
    actions = ["made a difficult decision", "took a big risk", "stood up for themselves", "admitted a mistake", "asked for help"]
    challenges = ["a serious illness", "financial difficulties", "family problems", "work stress", "personal loss"]
    feelings = ["invisible", "unheard", "misunderstood", "alone", "stuck", "powerless", "exhausted"]
    issues = ["self-doubt", "imposter syndrome", "grief", "anxiety", "depression", "stress", "loneliness"]
...
def generate_prompts_for_personality(personality: str, num_prompts: int) -> tuple[List[str], List[str]]:
    ...
    elif any(word in personality for word in ["empathetic", "caring", "compassionate", "emotional"]):
...
def main() -> None:
    ...
    parser.add_argument("--num-prompts", type=int, default=20, help="Number of prompts to generate for each set")
    ...
    print(f"Generated {len(positive_prompts)} positive prompts and {len(negative_prompts)} negative prompts")
