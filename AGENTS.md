...
- Record context, plan, risks, tests, and failures via 'bd' tool, mentioned at the bottom.
...
6. When user requests to update github, create a PR, use the CLI to merge it, sync ev'thing, get back to main and then confirm to the user.
...
**Data classes**
- **Secrets:** credentials, tokens, keys. It will and should ALWAYS be in .env file locally, that is the source of truth. Use it, add to it, edit it, as needed. 
...
## Appendix H — Repo quickstart and conventions (imported from AGENTS1)

### H.1 Project structure (persona-steering example)
...
> Treat this as a template for analogous repos. Keep library-only logic in `*/library` modules. Keep runnable demos in `scripts/`. Version data artifacts under `personas/` and `results/` when reproducible or intentionally shared.

### H.2 Build and run commands
...
> Map these into Interface Contract targets where relevant (`make setup`, `make test`, `make llm-live`). Keep HF cache via `TRANSFORMERS_CACHE` if needed.

### H.3 Coding style and naming
- Style: PEP 8, 4-space indents, type hints where practical.
- Names: modules/functions `snake_case`; classes `CapWords`.
- Personas: `personas/persona_<descriptor>.json` with matching `.pt`.
- Lint: if present, run `flake8`/`pylint`; format with Black.

> These sit under Section 2 Tiered Quality Gates. Do not weaken gates.

### H.4 Testing guidelines
- If no formal suite exists yet, validate via scripts above.
- For added tests, use `pytest`, place under `tests/`, name `test_*.py`.
...
### H.5 Commit and PR hygiene (examples)
- Messages: short, imperative. Examples: `scripts: add layer analysis helper`, `library: fix hook tuple output`.
- PRs: include summary, rationale, exact commands to reproduce, and result samples under `results/`.
- Link related issues. Keep diffs focused. Update README or docstrings when behavior changes.

> These augment Section 7 Git and Release Hygiene.

### H.6 Security and configuration
- Never commit secrets. Use `.env` locally; keep out of VCS.
- Large model downloads happen on first run. Document cache locations and quotas.
- Default backend PyTorch; MLX is experimental. Guard changes behind flags to avoid breaking torch path.

> These align with Appendix C security and supply chain. Add Semgrep policies as your repo matures.

### H.7 Runbook pointer (if applicable)
- Keep operational runbooks under `docs/` (e.g., `docs/CC_NEWS_4B_RUNBOOK.md`).
- Note: Some backends (e.g., `mlx_lm.generate`) can bypass hook injection; document safe flags (`--base-safe`, `--variants-safe`, `--hf-tokenizer`) to preserve intended behavior.

> Treat backend-specific caveats as part of Interface Contract notes and LLM live test goldens.
